...
def my_openai(api_key,client,assistant_id):  #please remove 
    ...
    thread = client.beta.threads.create(
        messages=[
            {   "role":"user",
                "content":"Please summarize the paragraphs."
            }
        ]
    )
    thread_id=thread.id
    ...
    if os.path.isfile(filename):
        ...
        for chunks in content_chunks:
            message = f"summarize in dot pints and group similar topic together: {chunks}"
